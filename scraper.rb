...
def scrape_list(url)
  noko = noko_for(url)
  section = noko.xpath('.//h3/span[@class="mw-headline" and contains(.,"Electorate Results")]')
  section.xpath('.//following::table[.//th[contains(.,"Candidate")]]').each do |table|
    constituency = table.css('tr').first.text[/2006: (.*)/, 1]
...
scrape_list('https://en.wikipedia.org/wiki/Cook_Islands_general_election,_2006')
